...
class BitstampSvcsController < ApplicationController
  ...
  def bal
      ...
      response = "{\"usd_balance\":\"100000\",\"btc_balance\":\""+Balances.where(:name => "Bitstamp").first.balance.to_s+"\",\"usd_reserved\":\"30000\",\"btc_reserved\":\"60\",\"usd_available\":\"100000\",\"btc_available\":\"200\",\"fee\":\"20\"}"
  ...
  def buy
      ...
      balB = Balances.where(:name => "Bitstamp").first
      balB.balance = balB.balance.to_f + params[:amount].to_f
      balB.save
      render :json => response
      end
  end 
