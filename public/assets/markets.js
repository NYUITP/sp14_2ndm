...
(function() {
  ...
  window.push_callback_hardcore = function(data) {
    var type_name;
    if (data["flash_info"] || data["ping"] || data["refresh"]) {
      push_callback(data);
    }
    if (data["trade"]) {
      push_hard([data["exchange_name"], data["currency"], "<span class=\"trade\">TRADE</span>", hard_desc("price:") + data["trade"]["price"], hard_desc("amount:") + data["trade"]["amount"]]);
    }
    if (data["ticker"]) {
      push_hard([data["exchange_name"], data["currency"], "TICKER", hard_desc("buy:") + data["ticker"]["buy"], hard_desc("sell:") + data["ticker"]["sell"]]);
    }
    if (data["depth"]) {
      type_name = (data["depth"]["type"] === "1" ? "ask" : "bid");
      push_hard([data["exchange_name"], data["currency"], "DEPTH", hard_desc("<span class=\"" + type_name + "\">" + type_name + "</span>" + " price") + data["depth"]["price"], hard_desc("amount") + data["depth"]["volume"]]);
    }
    if (data["depth_shot"]) {
      return push_hard([data["exchange_name"], data["currency"], "DEPTH CHART", hard_desc("what"), hard_desc("ever")]);
    }
  };

  hard_desc = function(x) {
    return "<span class=\"desc\">" + x + "</span> ";
  };

  window.push_hard = function(values) {
    var time_now;
    $("#hard_waiting").hide();
    time_now = new Date();
    $("#hard_table").append("<tr><td>" + hard_desc(time_now) + "</td><td>" + values[0] + "</td><td>" + values[1] + "</td><td>" + values[2] + "</td><td>" + values[3] + "</td><td>" + values[4] + "</td></tr>");
    return $("#hard_container").animate({
      scrollTop: $("#hard_table").height()
    }, 30);
  };

  window.cleanup_hardtable = function() {
    var i, s, _results;
    s = $("#hard_table tr").size();
    if (s > 2000) {
      i = 0;
      _results = [];
      while (i < s - 2000) {
        $("#hard_table tr").first().remove();
        _results.push(i = i + 1);
      }
      return _results;
    }
    ...
    var parseJSON = function(data) {
        ...
        if (window.JSON && window.JSON.parse) {
            try {
                ...
                throw "Invalid JSON window.parse: " + data;
            }
        }
        ...
        throw "Invalid JSON no test: " + data;
    };
...
(function() {
  window.Markets = {
    init: function() {
      Markets.start_time = new Date();
      Markets.last_update_time = new Date();
      Markets.js_version = '7';
      try {
        if ($('#placeholder').length > 0) {
          Markets.price_chart.init();
        }
      } catch (_error) {}
      try {
        if ($('#depth_placeholder').length > 0) {
          Markets.depth_chart.init();
        }
      } catch (_error) {}
      Markets.fadeTrades();
      setInterval("Markets.fadeTrades()", 2000);
      setInterval("Markets.refresh_last_update()", 1000);
      Markets.set_title();
      Markets.init_prefs_dialog();
      Markets.init_sound_prefs_dialog();
      if (Markets.hardcore_mode != null) {
        Markets.init_hardcore();
      } else {
        Markets.init_pushstream();
      }
      Markets.theme_specific();
      if (Markets.options.sound_enabled === 'yes') {
        return Markets.sound.init();
      }
    },
    sound: {
      init: function() {
        if (Markets.sound.enabled) {
          return;
        }
        return $.getScript('/assets/soundmanager2.min.js', function() {
          soundManager.setup({
            url: '/sm_swf/',
            ontimeout: function(foo) {
              if (!Markets.sound.init_retried) {
                Markets.sound.init.retried = true;
                return setTimeout("window.Markets.sound.init()", 2000);
              }
            },
            onready: function() {
              Markets.sound.enabled = true;
          ...
          Markets.sound.last_price = Markets.current_price();
          Markets.sound.volume_counter = 0;
          Markets.sound.play_queue = [];
          Markets.sound.last_play = new Date();
          Markets.sound.queue = [];
          Markets.sound.locked = false;
          Markets.sound.volume_threshold = parseFloat(Markets.options.sound_volume_threshold);
          return Markets.sound.enabled_trades = Markets.options.sound_enabled_trades === 'yes';
        });
      },
      tick: function(price, volume) {
        var new_volume, trend;
        if (!Markets.sound.enabled) {
          return;
        }
        Markets.sound.price_alert_check(price);
        if (Markets.sound.enabled_trades) {
          Markets.sound.volume_counter += volume;
          if (Markets.sound.volume_counter > Markets.sound.volume_threshold) {
            if (Markets.sound.last_price < price) {
              trend = 'up';
            }
            if (Markets.sound.last_price === price) {
              trend = 'flat';
            ...
            Markets.sound.last_price = price;
            Markets.sound.play(trend);
            new_volume = Markets.sound.volume_counter - Markets.sound.volume_threshold;
            Markets.sound.volume_counter = 0;
            return Markets.sound.tick(price, new_volume);
          }
        }
      },
      form_switch: function() {
        if ($('#sound_prefs_form #sen').val() === 'yes') {
          Markets.sound.init();
          $('#sound_prefs_form .detail').find('select, input').removeAttr('disabled');
          if ($('#sound_prefs_form select.sound_alert').val() === 'yes') {
            return $('#sound_prefs_form .detail.price_alert').find('select, input').removeAttr('disabled');
          } else {
            return $('#sound_prefs_form .detail.price_alert').find('select, input').attr('disabled', 'disabled');
          }
        } else {
          return $('#sound_prefs_form .detail').find('select, input').attr('disabled', 'disabled');
        }
      },
      check_desktop_notifications: function() {
        if (window.webkitNotifications.checkPermission() !== 0) {
          window.webkitNotifications.requestPermission();
        }
        return Markets.options.desktop_notification = $('#sound_prefs_form #san').val();
      },
      price_alert: function(trend, price) {
        var params, time_now;
        Markets.options.sound_alert_enabled = 'no';
        params = {
          alert_fired: true,
          trend: trend,
          price: price
        };
        params['sa' + Markets.currency] = 'no';
        $.post("/markets/save_prefs", params);
        $('#price_alert_dialog .currency').html(Markets.currency);
        $('#price_alert_dialog .trend_name').html(trend);
        $('#price_alert_dialog .price').html(price);
        time_now = new Date();
        $('#price_alert_dialog .time').html(time_now.toLocaleTimeString());
        Markets.sound.play('alert_' + trend);
        Markets.price_alert_interval = setInterval("window.Markets.sound.play('alert_" + trend + "')", 7000);
        $("#price_alert_dialog").dialog({
          modal: true,
          width: 500,
          resizable: false,
          close: function() {
            if (Markets.sound.desktop_notification) {
              Markets.sound.desktop_notification.cancel();
            }
            return clearInterval(Markets.price_alert_interval);
          },
          buttons: {
            Ok: function() {
        ...
        if (Markets.sound.desktop_notification_enabled()) {
          Markets.sound.desktop_notification = window.webkitNotifications.createNotification('/assets/bitcoin_icon.png', 'Bitcoinity alert', 'Price went ' + trend + ' to ' + price);
          Markets.sound.desktop_notification.onclose = function() {
            return $("#price_alert_dialog").dialog("close");
          };
          return Markets.sound.desktop_notification.show();
        }
      },
      desktop_notification_enabled: function() {
        return window.webkitNotifications && Markets.options.desktop_notification === 'yes';
      },
      price_alert_check: function(price) {
        if (Markets.options.sound_alert_enabled === 'yes') {
          if (Markets.options.sound_alert_up !== 0) {
            if (Markets.options.sound_alert_up <= price) {
              Markets.sound.price_alert('up', Markets.options.sound_alert_up);
            }
          }
          if (Markets.options.sound_alert_down !== 0) {
            if (Markets.options.sound_alert_down >= price) {
              return Markets.sound.price_alert('down', Markets.options.sound_alert_down);
            }
          }
        }
      },
      play: function(trend) {
        var play_sound, time_now;
        if (!Markets.sound.enabled) {
          return;
        }
        if (trend != null) {
          Markets.sound.queue.unshift(trend);
        }
        if (Markets.sound.locked === true) {
          time_now = new Date();
          if ((time_now - Markets.sound.last_play) > 1000) {

          } else {
            return;
          }
        }
        Markets.sound.locked = true;
        Markets.sound.last_play = new Date();
        play_sound = Markets.sound.queue.pop();
        if (!play_sound) {
          Markets.sound.locked = false;
          return;
        }
        return soundManager.play(play_sound, {
          onfinish: function() {
            Markets.sound.locked = false;
            if (Markets.sound.queue.length > 0) {
              return Markets.sound.play();
            }
          }
        });
      }
    ...
    pushstream: new PushStream({
      ...
      checkChannelAvailabilityInterval: 5000
    }),
    init_pushstream: function() {
      var messageReceived, prefix;
      Markets.pushstream.onmessage = messageReceived = function(text, id, channel) {
        return Markets.push_callback(text);
      };
      Markets.pushstream.addChannel("markets");
      if (Markets.mobile_version) {
        prefix = 'markets_mobile_';
      } else {
        prefix = 'markets_';
      }
      Markets.pushstream.addChannel("chat");
      Markets.pushstream.addChannel(prefix + Markets.exchange + "_" + Markets.currency, Markets.push_callback);
      return Markets.pushstream.connect();
    },
    init_hardcore: function() {
      var messageReceived;
      Markets.pushstream.onmessage = messageReceived = function(text, id, channel) {
        return push_callback_hardcore(text);
      };
      Markets.pushstream.addChannel("stats");
      Markets.pushstream.addChannel("chat");
      Markets.pushstream.connect();
      setInterval("cleanup_hardtable()", 300000);
      return Markets.btc_theme = "dark";
    },
    push_callback: function(data) {
      var amount, current, el, evenodd, exchange, first, i, lag, last, last_one, last_price, new_row, prev, prev_amount, prev_last, price, s, table, time_now, time_now_i, trend, trp1, trp2, volume, wait_time;
      Markets.last_update_time = new Date();
      time_now = new Date();
      time_now_i = time_now / 1000;
      if (data["chat"]) {
        if (data["chat"]["channel"] === "btce_chat_en") {
          table = $('#chat_messages');
          first = table.find('tr:first-child');
          last = table.find("tr:last-child");
          prev_last = last.prev();
          trp1 = last.clone();
          trp2 = prev_last.clone();
          last.remove();
          prev_last.remove();
          first.after(trp1);
          trp1.find('.message').html(data["chat"]["msg"]);
          first.after(trp2);
          trp2.find('.timeago').attr('data-livestamp', '' + time_now_i);
          trp2.find('.user').html(data["chat"]["user"]);
          $.livestamp.update();
        }
      }
      if (data["flash_info"]) {
        if (data["flash_info"] === "-") {
          $("#flash").slideUp();
          $("#flash .info").html(data["flash_info"]);
        } else {
          $("#flash .info").html(data["flash_info"]);
          $("#flash").slideDown();
        }
      ...
      if (data["ping"]) {
        if (data["wait_time"]) {
          wait_time = parseFloat(data["wait_time"]);
        } else {
          wait_time = 60;
        }
        setTimeout("Markets.do_pong('" + data["ping"] + "');", Math.random() * wait_time * 1000);
      }
      if (data["reload"]) {
      ...
      if (data["subtitle"]) {
        $(".subtitle").fadeOut(function() {
          return $(this).html(data["subtitle"]);
        }).fadeIn();
      }
      if (data["connected_count"]) {
        $(".connected_users").html(data["connected_count"]);
      }
      if (data["leaves"] === "yes") {
        window.stopFallingLeaves = 0;
        leavesInit();
        fallingLeaves();
        setTimeout("window.stopFallingLeaves = 1; $(\".leave\").remove()", 60 * 1000);
      }
      if (data["leaves"] === "no") {
        window.stopFallingLeaves = 1;
        $(".leave").remove();
      }
      if (data["mtgox_lag"]) {
        lag = parseFloat(data["mtgox_lag"]);
        if (Markets.options.glag_display !== "n") {
          if (lag < 1.0 && Markets.options.glag_display !== "a") {
            $("#gox_lag").fadeOut(400, function() {
              return $("#last_trades tr").last().show();
            });
          } else {
            $("#last_trades tr").last().hide();
            if (lag > 5.0) {
              $("#gox_lag").removeClass("light");
            } else {
              $("#gox_lag").addClass("light");
            }
            $("#gox_lag").fadeIn();
            if (lag > 60) {
              lag = Math.round(lag);
              $("#gox_lag .value").html(Math.floor(lag / 60) + "m " + (lag % 60) + "s");
            } else {
              $("#gox_lag .value").html(data["mtgox_lag"] + "s");
            }
          }
        }
      }
      if (data["currency"] === Markets.options.filter_currency && ((data["last"] != null) || ((data["ticker"] != null) && (data["ticker"]["last"] != null)))) {
        if ($(".top_box.exchange." + data["exchange_name"]).length > 0) {
          prev = parseFloat($(".top_box.exchange." + data["exchange_name"] + " .price.last").html());
          current = 0.0;
          if (data["last"] != null) {
            current = data["last"];
          } else {
          ...
          el = $(".top_box.exchange." + data["exchange_name"] + " .price.last");
          el.html(current.toFixed(2));
          if (prev > current) {
            el.addClass("down");
            el.removeClass("up");
          } else {
      ...
      if (!(Markets.exchange !== "" ? data["exchange_name"] === Markets.exchange : void 0)) {
        return false;
      }
      if (data["currency"] !== Markets.currency) {
        return false;
      }
      if (data["all_time_high"] === "yes") {
        $(".all_time_high").fadeIn();
        if (Markets.btc_theme === "dark") {
          $(".all_time_high").animate({
            color: "#ff0"
          }, 1000).animate({
            color: "#fff"
          }, 1000);
      ...
      if (data["all_time_high"] === "no") {
        $(".all_time_high").stop(true).hide();
        $(".all_time_high").css("opacity", "1");
      }
      if (data["ticker"]) {
        data["ticker"]["last"] != null;
        $("#last_sell").html(data["ticker"]["sell"].toFixed(Markets.options.lt_price_fix));
        $("#last_buy").html(data["ticker"]["buy"].toFixed(Markets.options.lt_price_fix));
      }
      if (data["trade"]) {
        last_one = $("#last_trades tr").first();
        last_price = parseFloat($(last_one.children("td").get(3)).text());
        price = data["trade"]["price"].toFixed(Markets.options.lt_price_fix);
        volume = data["trade"]["amount"];
        if (mixed_markets === 1) {
          Markets.price_chart.lasts[data["exchange_name"]].push([data["trade"]["date"] * 1000, price]);
          $("#last_price_" + data["exchange_name"]).html(data["trade"]["price"].toFixed(Markets.options.lt_price_fix));
        } else {
          Markets.price_chart.push_trade(data["trade"]);
          $("#last_price").html(data["trade"]["price"].toFixed(Markets.options.lt_price_fix));
        }
        Markets.set_title();
        Markets.sound.tick(price, volume);
        if (data["trade"]["amount"] > Markets.options.fivo) {
          if (last_price === parseFloat(price)) {
            prev_amount = parseFloat($(last_one.children("td").get(1)).html());
            $(last_one.children("td").get(1)).html((prev_amount + data["trade"]["amount"]).toFixed(2));
            $(last_one.children("td").get(2)).html("<abbr class=\"timeago\" data-livestamp=\"" + time_now_i + "\"></abbr>");
            last_one.attr("time", time_now);
            $.livestamp.update();
          } else {
            evenodd = (last_one.attr("class") === "odd" ? "even" : "odd");
            amount = data["trade"]["amount"].toFixed(2);
            exchange = data["trade"]["exchange_name"];
            trend = (last_price > parseFloat(price) ? "down" : "up");
            new_row = $("#last_trades").prepend("<tr class=\"" + evenodd + "\" time=\"" + time_now + "\"><td class=\"exchange_name\">" + exchange + "</td><td align=\"right\">" + amount + "</td><td class=\"time_ago\"><abbr class=\"timeago\" data-livestamp=\"" + time_now_i + "\"></abbr></td><td align=right>" + price + "&nbsp;<img src=\"/assets/stats/trend_" + trend + ".gif\" class=\"trend\"></td></tr>");
            $.livestamp.update();
            s = $("#last_trades tr").size();
            if (s > 20) {
              i = 0;
              while (i < s - 20) {
                $("#last_trades tr").last().remove();
                i = i + 1;
              }
            } else {

            }
            if ($("#gox_lag").is(":visible")) {
              $("#last_trades tr").last().hide();
            }
          }
        }
      ...
      if (data["depth_shot"]) {
        if (mixed_markets === 1) {
          if (data["exchange_name"] !== "all") {
            return false;
          }
        }
        Markets.depth_chart.add_depth_shot(data);
      }
      if (data["depth"]) {
        return Markets.depth_chart.add_depth(data["depth"]["price"], data["depth"]["volume"], data["depth"]["type"]);
      }
    },
    format_number: function(number) {
      ...
      return parseFloat(number.toFixed(round)) + scale;
    },
    fadeTrades: function() {
      var time_now;
      time_now = new Date();
      return $("#last_trades tr").each(function(idx) {
        var its_time, minopac, opac;
        its_time = new Date($(this).attr("time"));
        opac = 1 / ((time_now - its_time) / 300000.0);
        minopac = 0.4;
        opac = (opac * (1 - minopac) + minopac).toFixed(2);
        return $(this).find("td").fadeTo(0, opac);
      });
    },
    refresh_last_update: function() {
      var diff, time_now;
      time_now = new Date();
      diff = (time_now - Markets.last_update_time) / 1000;
      return $("#last_update").html(diff.toFixed(2));
    },
    current_price: function() {
      return parseFloat($("#last_price").html());
    },
    ...
    set_title: function() {
      return document.title = Markets.current_price().toFixed(2) + " " + Markets.currency + "/BTC";
    },
    do_pong: function(ping) {
      var time_now;
      time_now = new Date();
      return $.post("/markets/pong", {
        ping: ping,
        exchange_name: Markets.exchange,
        currency: Markets.currency,
        time_on_page: Math.round(time_now / 1000 - Markets.start_time),
        mkts_version: Markets.js_version,
        theme: Markets.btc_theme,
        timespan: $(".time_links span.active:visible").text().replace(/^\s+|\s+$/g, ""),
        protocol: window.location.protocol
      });
    },
    active_time_link: function(name) {
      $(".time_links .active").hide();
      $(".time_links .inactive").show();
      $(".time_links .active." + name).show();
      return $(".time_links .inactive." + name).hide();
    },
    active_bars_link: function(name) {
      $(".bars_links .active").hide();
      $(".bars_links .inactive").show();
      $(".bars_links .active." + name).show();
      return $(".bars_links .inactive." + name).hide();
    },
    dot_ping: function() {
      if (Markets.btc_theme === "light") {
        $("#donation_info").animate({
          height: ["toggle", "swing"],
          color: "#000"
        }, 5000);
        $("#donation_info").animate({
    ...
    init_prefs_dialog: function() {
      return $("#prefs_dialog").dialog({
        autoOpen: false,
        width: 400,
        height: 280,
        modal: true,
    ...
    init_sound_prefs_dialog: function() {
      if (!window.webkitNotifications) {
        $('#sound_prefs_dialog tr.desktop_notification').hide();
      }
      return $("#sound_prefs_dialog").dialog({
        autoOpen: false,
        width: 540,
        modal: true,
        buttons: {
          "Apply and save": function() {
            return $("#sound_prefs_form").submit();
          },
          Cancel: function() {
            return $(this).dialog("close");
          }
        ...
        title: "Preferences",
        open: function() {
          return Markets.sound.form_switch();
        }
      });
    },
    theme_specific: function() {
      if (Markets.btc_theme === "dark") {
        $('.right_info img').each(function(idx, val) {
        ...
        return $.each([".right_info", ".middle_top"], function(idx, val) {
          $(val).mouseenter(function() {
            $(val).animate({
              color: "#aaa"
            }, 200);
            return $(val + " a").animate({
              color: "#ddd"
            }, 200);
          });
          return $(val).mouseleave(function() {
            $(val).animate({
              color: "#666"
            }, 50);
            return $(val + " a").animate({
...
(function() {
  Markets.price_chart = {
    init: function() {
      setInterval("Markets.price_chart.ticker_move()", 5000);
      this.flot_plot = $.plot($("#placeholder"), ticker_plot_data, {
        xaxis: {
          mode: "time",
        ...
        yaxes: [
          {
            ...
            tickFormatter: function(number) {
              return Markets.format_number(number);
            }
          }
        ],
        grid: {
        ...
        legend: {
          ...
          labelBoxBorderColor: "rgb(80,80,80)",
          margin: 4
        }
      });
      return this.enabled = true;
    },
    push_trade: function(trade) {
      var price, price_str, trade_time;
      if (this !== Markets.price_chart) {
        return Markets.price_chart.push_trade.call(Markets.price_chart, trade);
      }
      if (!this.enabled) {
        return;
      }
      trade_time = trade["date"] * 1000;
      price_str = trade["price"].toFixed(Markets.options.lt_price_fix);
      price = parseFloat(price_str);
      if (this.lasts[this.lasts.length - 1][0] > (trade_time - (this.timespan / 70) * 1000)) {
        this.lasts[this.lasts.length - 1][1] = price;
        if (price > this.buys[this.buys.length - 1][1]) {
          this.buys[this.buys.length - 1][1] = price;
        }
        if (price < this.sells[this.sells.length - 1][1]) {
          this.sells[this.sells.length - 1][1] = price;
        }
      } else {
        this.lasts.push([trade_time, price]);
        this.buys.push([trade_time, price]);
        this.sells.push([trade_time, price]);
      }
      this.sort_by_time(this.lasts);
      if (price < parseFloat($(".price.low.value").html())) {
        $(".price.low.value").html(trade["price"].toFixed(2));
        $(".price.low.value").css("background-color", Markets.page_bg_color());
        $(".price.low.value").animate({
          "background-color": "#800"
        }, 500).animate({
          "background-color": Markets.page_bg_color()
        });
      }
      if (price > parseFloat($(".price.high.value").html())) {
        $(".price.high.value").html(trade["price"].toFixed(2));
        $(".price.high.value").css("background-color", Markets.page_bg_color());
        $(".price.high.value").animate({
          "background-color": "#080"
        }, 500).animate({
          "background-color": Markets.page_bg_color()
        });
      }
      return this.add_volume(trade["amount"], trade_time);
    },
    add_volume: function(amount, time_now) {
      var k, li;
      k = ((time_now / 1000) - ((time_now / 1000) % Markets.price_chart.volume_resolution)) * 1000;
      li = Markets.price_chart.volume.length - 1;
      if (Markets.price_chart.volume[li][0] === k) {
        return Markets.price_chart.volume[li][1] = Markets.price_chart.volume[li][1] + amount;
      } else {
        return Markets.price_chart.volume.push([k, amount]);
      }
    },
    filter_old_values: function(t) {
      var now_time, _results;
      now_time = new Date().getTime();
      try {
        _results = [];
        while (now_time - t[0][0] > this.timespan * 1000) {
          _results.push(t.shift());
        }
        return _results;
      } catch (_error) {}
    },
    make_sure_volume_uniq: function(a) {
      var i;
      i = 1;
      while (i < a.length) {
        if (a[i - 1][0] === a[i][0]) {
    ...
    ticker_move: function() {
      if (Markets.price_chart !== this) {
        return Markets.price_chart.ticker_move.call(Markets.price_chart);
      }
      if (mixed_markets === 1) {
        this.filter_old_values(this.lasts["mtgox"]);
        ticker_plot_data[1]["data"] = this.lasts["mtgox"];
        this.filter_old_values < lasts["tradehill"];
        ticker_plot_data[2]["data"] = this.lasts["tradehill"];
      } else {
        this.filter_old_values(this.lasts);
        ticker_plot_data[1]["data"] = this.lasts;
        this.filter_old_values(this.sells);
        ticker_plot_data[2]["data"] = this.sells;
        this.filter_old_values(this.buys);
        ticker_plot_data[3]["data"] = this.buys;
      }
      this.sort_by_time(this.volume);
      this.filter_old_values(this.volume);
      this.make_sure_volume_uniq(this.volume);
      ticker_plot_data[0]["data"] = this.volume;
      this.flot_plot.setData(ticker_plot_data);
      this.flot_plot.setupGrid();
      return this.flot_plot.draw();
    },
    load_ticker_plot: function(d, t, x) {
      if (Markets.price_chart !== this) {
        return Markets.price_chart.load_ticker_plot.call(Markets.price_chart, d, t, x);
      }
      $(".price_chart_top_right .high.value").html(d["price_high"]);
      $(".price_chart_top_right .low.value").html(d["price_low"]);
      if (mixed_markets === 1) {
        this.lasts = d["mixed_lasts"];
      } else {
        this.sells = d["sells"];
        this.buys = d["buys"];
        this.lasts = d["lasts"];
      ...
      this.timespan = d["ticker_life"];
      this.volume_resolution = d["volume_resolution"];
      ticker_plot_data[0]["bars"]["barWidth"] = Math.round((this.volume_resolution * 0.7) * 1000);
      return this.ticker_move();
    }
  };
...
(function() {
  Markets.depth_chart = {
    init: function() {
      this.depth_ents = [];
      this.flot_data[0]['bars']['barWidth'] = this.depth_change_resolution * 0.5;
      this.flot_data[1]['bars']['barWidth'] = this.depth_change_resolution * 0.5;
      this.flot_plot = $.plot($("#depth_placeholder"), this.flot_data, this.flot_options());
      this.set_zoom(this.zoom);
      this.flot_plot.setupGrid();
      this.flot_plot.draw();
      this.enable_tooltip();
      $.getJSON('/markets/get_depth_change?currency=' + Markets.currency + '&exchange=' + Markets.exchange + '&dc_span=10m', Markets.depth_chart.load_depth_change);
      setInterval("Markets.depth_chart.compute_depth_change()", 20000);
      this.enabled = true;
      return true;
    },
    flot_options: function() {
      var depth_plot_options;
      depth_plot_options = {
        yaxes: [
          {
            alignTicksWithAxis: "right",
      ...
      if (Markets.options.dcs === "log") {
        $.extend(depth_plot_options.yaxes[0], {
          min: -10000,
          max: 10000,
          transform: function(v) {
            ...
            if (v > 0) {
              ...
              return -1 * Math.log(-v) / Math.log(10);
            }
          },
      ...
      return depth_plot_options;
    },
    add_depth_shot: function(data) {
      if (Markets.depth_chart !== this) {
        return Markets.depth_chart.add_depth_shot.call(Markets.depth_chart, data);
      }
      if (!this.enabled) {
        return;
      }
      c_depths.unshift(data["depth_shot"]);
      c_depths.pop();
      this.flot_data[2]["data"] = c_depths[3][1];
      this.flot_data[3]["data"] = c_depths[3][0];
      this.flot_data[4]["data"] = c_depths[2][1];
      this.flot_data[5]["data"] = c_depths[2][0];
      this.flot_data[6]["data"] = c_depths[1][1];
      this.flot_data[7]["data"] = c_depths[1][0];
      this.flot_data[8]["data"] = c_depths[0][1];
      this.flot_data[9]["data"] = c_depths[0][0];
      this.flot_plot.setData(this.flot_data);
      this.set_zoom(this.zoom);
      this.flot_plot.setupGrid();
      return this.flot_plot.draw();
    },
    adjust_y: function() {
      var maxv1, maxv2, nmax, nmin;
      maxv1 = 0;
      maxv2 = 0;
      nmax = Markets.depth_chart.flot_plot.getOptions()["xaxes"][0]["max"];
      nmin = Markets.depth_chart.flot_plot.getOptions()["xaxes"][0]["min"];
      $.each(c_depths[0][0], function(idx, val) {
        var v, x;
        x = val[0];
        v = val[1];
        if (x > nmin) {
      ...
      $.each(c_depths[0][1], function(idx, val) {
        var v, x;
        x = val[0];
        v = val[1];
        if (x < nmax) {
      ...
      Markets.depth_chart.flot_plot.getOptions()["yaxes"][1]["max"] = Math.max(maxv1, maxv2);
      return Markets.depth_chart.flot_plot.getOptions()["yaxes"][1]["min"] = 0;
    },
    center: function() {
      var cmax, cmin, nmax, nmin, price, space_left, space_right;
      cmax = Markets.depth_chart.flot_plot.getAxes()["xaxis"]["max"];
      cmin = Markets.depth_chart.flot_plot.getAxes()["xaxis"]["min"];
      price = parseFloat($("#last_price").html());
      space_left = price - cmin;
      space_right = cmax - price;
      nmax = void 0;
      nmin = void 0;
      if (space_left > space_right) {
        nmax = price + space_left;
      } else {
        nmin = price - space_right;
      }
      Markets.depth_chart.flot_plot.getOptions()["xaxes"][0]["max"] = nmax;
      return Markets.depth_chart.flot_plot.getOptions()["xaxes"][0]["min"] = nmin;
    },
    change_zoom: function(how) {
      var cmax, cmin, nmax, nmin;
      if (Markets.depth_chart !== this) {
        return Markets.depth_chart.call.change_zoom(Markets.depth_chart, how);
      }
      this.center();
      cmax = this.flot_plot.getAxes()["xaxis"]["max"];
      cmin = this.flot_plot.getAxes()["xaxis"]["min"];
      if (how === "in") {
        nmax = cmax - (cmax - cmin) * 0.1;
        nmin = cmin + (cmax - cmin) * 0.1;
      } else if (how === "out") {
        nmax = cmax + (cmax - cmin) * 0.1;
        nmin = cmin - (cmax - cmin) * 0.1;
      }
      this.zoom = ((nmax - nmin) / 2.0) / Markets.current_price();
      this.flot_plot.getOptions()["xaxes"][0]["max"] = nmax;
      this.flot_plot.getOptions()["xaxes"][0]["min"] = nmin;
      this.adjust_y();
      this.flot_plot.setupGrid();
      this.flot_plot.draw();
      this.depth_change_resolution = (nmax - nmin) / 40.0;
      this.flot_data[0]["bars"]["barWidth"] = this.depth_change_resolution * 0.5;
      this.flot_data[1]["bars"]["barWidth"] = this.depth_change_resolution * 0.5;
      this.flot_plot.setData(this.flot_data);
      return this.compute_depth_change();
    },
    set_zoom: function(zoom) {
      var nmax, nmin, price;
      price = parseFloat($("#last_price").html());
      nmax = price + price * zoom;
      nmin = price - price * zoom;
      this.flot_plot.getOptions()["xaxes"][0]["max"] = nmax;
      this.flot_plot.getOptions()["xaxes"][0]["min"] = nmin;
      Markets.depth_chart.depth_change_resolution = (nmax - nmin) / 40.0;
      this.flot_data[0]["bars"]["barWidth"] = Markets.depth_chart.depth_change_resolution * 0.5;
      this.flot_data[1]["bars"]["barWidth"] = Markets.depth_chart.depth_change_resolution * 0.5;
      return this.adjust_y();
    },
    load_depth_change: function(d, t, x) {
      this.depth_ents = [];
      $.each(d["data"], function(idx, val) {
        return Markets.depth_chart.add_depth(val["price"], val["volume"], val["depth_type"], val["created_at"] * 1000);
      });
      Markets.depth_chart.depth_change_span = d["span"];
      return Markets.depth_chart.compute_depth_change();
    },
    add_depth: function(price, amount, dt, time) {
      var time_now;
      if (!this.enabled) {
        return;
      }
      ...
      return this.depth_ents.push([time_now.getTime(), price, amount, dt]);
    },
    live_depth_update: function(price, amount, dt) {
      var i, j, prev;
      if (dt === 2) {
        i = c_depths[0][0].length - 1;
        prev = void 0;
        if (price > c_depths[0][0][i][0]) {
          c_depths[0][0].push([price, amount]);
        }
        while (i >= 0) {
          if (c_depths[0][0][i][0] <= price) {
            c_depths[0][0][i][1] += amount;
          }
          if ((prev != null) && c_depths[0][0][i][1] < prev) {
            j = i + 1;
            while (j < c_depths[0][0].length) {
              if (c_depths[0][0][j][1] > c_depths[0][0][i][1]) {
                c_depths[0][0][j][1] = c_depths[0][0][i][1];
              } else {
                break;
              }
              j++;
          ...
          prev = c_depths[0][0][i][1];
          i--;
        }
        while (c_depths[0][0][c_depths[0][0].length - 1][1] <= 0) {
          c_depths[0][0].pop();
        }
      }
      if (dt === 1) {
        prev = void 0;
        i = 0;
        if (price < c_depths[0][1][0][0]) {
          c_depths[0][1].unshift([price, amount]);
        }
        while (i < c_depths[0][1].length) {
          if (c_depths[0][1][i][0] >= price) {
            c_depths[0][1][i][1] += amount;
          }
          if ((prev != null) && c_depths[0][1][i][1] < prev) {
            j = i - 1;
            while (j >= 0) {
              if (c_depths[0][1][j][1] > c_depths[0][1][i][1]) {
                c_depths[0][1][j][1] = c_depths[0][1][i][1];
              } else {
                break;
              }
              j--;
          ...
          prev = c_depths[0][1][i][1];
          i++;
        }
        while (c_depths[0][1][0][1] <= 0) {
          c_depths[0][1].shift();
        }
      }
      return Markets.depth_chart.must_replot_depth = true;
    },
    replotter: function() {
      if (Markets.depth_chart.must_replot_depth === false) {
        return false;
      }
      Markets.depth_chart.must_replot_depth = false;
      Markets.depth_chart.flot_data[8]["data"] = c_depths[0][1];
      Markets.depth_chart.flot_data[9]["data"] = c_depths[0][0];
      Markets.depth_chart.flot_plot.setData(Markets.depth_chart.flot_data);
      Markets.depth_chart.set_zoom(Markets.depth_chart.zoom);
      Markets.depth_chart.flot_plot.setupGrid();
      Markets.depth_chart.flot_plot.draw();
      return true;
    },
    compute_depth_change: function() {
      var bars, cmax, cmin, depth_changes_down, depth_changes_up, i, maxbar, new_depth_ents, now_time, v;
      if (Markets.depth_chart.flot_plot === undefined) {
        return false;
      }
      bars = {
        1: {},
      ...
      new_depth_ents = [];
      now_time = new Date().getTime();
      $.each(this.depth_ents, function(idx, val) {
        if (now_time - val[0] < Markets.depth_chart.depth_change_span * 1000) {
          return new_depth_ents.push(val);
        }
      });
      this.depth_ents = new_depth_ents;
      $.each(this.depth_ents, function(idx, val) {
        var amount, dt, k, price;
        price = val[1];
        amount = val[2];
        dt = val[3];
        k = Math.round(price / Markets.depth_chart.depth_change_resolution) * Markets.depth_chart.depth_change_resolution;
        if (bars[dt][k] !== undefined) {
          return bars[dt][k] = bars[dt][k] + amount;
        } else {
          return bars[dt][k] = val[2];
        }
      });
      depth_changes_up = new Array();
      depth_changes_down = new Array();
      cmin = Markets.depth_chart.flot_plot.getAxes()["xaxis"]["min"];
      cmax = Markets.depth_chart.flot_plot.getAxes()["xaxis"]["max"];
      maxbar = 0;
      for (i in bars[1]) {
        if (i > cmin && i < cmax) {
          v = bars[1][i];
          if (Math.abs(v) > maxbar) {
            maxbar = Math.abs(v);
          }
          if (v < 0) {
            depth_changes_up.push([i, v]);
          } else {
            depth_changes_down.push([i, v]);
          }
        }
      }
      for (i in bars[2]) {
      ...
      Markets.depth_chart.flot_data[0]["data"] = depth_changes_up;
      Markets.depth_chart.flot_data[1]["data"] = depth_changes_down;
      Markets.depth_chart.flot_plot.setData(Markets.depth_chart.flot_data);
      Markets.depth_chart.flot_plot.setupGrid();
      return Markets.depth_chart.flot_plot.draw();
    },
    enable_tooltip: function() {
      return $("#depth_placeholder").bind("plothover", function(event, pos, item) {
        var dollars, ds, prev_val, previousPoint, x, y, yv;
        ...
        if (item) {
          if (previousPoint !== item.dataIndex) {
            ...
            yv = item.datapoint[1];
            if (yv > 10000) {
              y = (item.datapoint[1] / 1000.0).toFixed(2) + "k";
            } else {
              if (yv < 10) {
                y = yv.toFixed(2);
              } else {
            ...
            try {
              if (item.series.depth_type === "asks") {
                dollars = 0;
                prev_val = 0;
                $.each(item.series.data, function(idx, val) {
                  var now_val;
                  if (val[0] > x) {
                    return false;
                  }
                  now_val = val[1] - prev_val;
                  prev_val = val[1];
                  return dollars = dollars + val[0] * now_val;
                });
                ds = dollars.toFixed(2);
                if (dollars > 1000) {
                  ds = (dollars / 1000.0).toFixed(2) + "k";
                }
                if (dollars > 1000000) {
                  ds = (dollars / 1000.0 / 1000.0).toFixed(2) + "M";
                }
                return Markets.showTooltip(item.pageX, item.pageY, "&nbsp;&nbsp;<b>" + y + " BTC</b> can be bought<br />&nbsp;&nbsp;for total of <b>" + ds + " " + Markets.currency + "</b><br />&nbsp;&nbsp;until price goes above <b>" + x + "</b>");
              } else {
                dollars = 0;
                prev_val = 0;
                item.series.data.reverse();
                $.each(item.series.data, function(idx, val) {
                  var now_val;
                  if (val[0] < x) {
                    return false;
                ...
                return Markets.showTooltip(item.pageX, item.pageY, "&nbsp;&nbsp;<b>" + y + " BTC</b> can be sold<br />&nbsp;&nbsp;for total of <b>" + ds + " " + Markets.currency + "</b><br />&nbsp;&nbsp;until price goes below <b>" + x + "</b>");
              }
            } catch (_error) {}
          }
        } else {
...
// XXX falling $100 require leaves_falling



  /*!
   * (c) 2011 Kacper Cieśla. All rights reserved
   * */
...
$(document).ready(function() {
    ...
    var container = chat_messages.parent().parent();

    container.draggable({ addClasses: false, cursor: "move"});

    container.resizable({ animate: false });
